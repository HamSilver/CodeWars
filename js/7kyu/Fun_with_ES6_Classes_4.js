...
class Cube {
  constructor (len) { this.len = len; }
  
  set length(len) { this.len = len; }
  get length() { return this.len; }  
  
  set surfaceArea(area) { this.len = Math.sqrt(area / 6); }
  get surfaceArea() { return this.len * this.len * 6; }
  
  set volume(vol) { this.len = Math.cbrt(vol); }
  get volume() { return this.len ** 3; }
}

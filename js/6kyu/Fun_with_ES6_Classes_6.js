...
class File {
  ...
  set fullName(foo = null) {  }
  get fullName() { return this.full; }
  
  set filename(foo = null) {  }
  get filename() {
    const ext = path.extname(this.full);
    return path.basename(this.full, ext);
  }
  
  set extension(foo = null) { }
  get extension() {
    const ext = path.extname(this.full);
    return ext.replace(/^\./, '');
  }
  
  getContents() { return this.file; }
  ...
  gets() {
    ...
    if (this.offsetS < lines.length) {
      this.offsetS++;
      return lines[this.offsetS - 1];
    } else return;
  }

  getc() {
    if (this.offsetC < this.file.length) {
      this.offsetC++;
      return this.file[this.offsetC-1];
    } else return;
  }
